...
You can provide your own encryption secret using environment variables or the runtime configuration. This is useful when you need deterministic keys across deployments or when running multiple server instances.
...
3. `serverFunctions.secret` in the runtime configuration
4. `serverFunctions.secretFile` in the runtime configuration (path to a key file)
