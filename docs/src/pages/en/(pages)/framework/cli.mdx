...
**mode:** Set the mode for the build. Default is `production`.

**edge:** Enable edge build mode. Default is `false`.
Builds your app for edge runtime environments.

**no-ssr-worker:** Disable SSR worker. Default is `false`. When enabled, the server-side rendering will be done in a worker thread. You can disable this if you want to run the server-side rendering in the main thread.

**silent:** Suppress build output. Default is `false`.  
...
**verbose:** Verbose build output. Default is `false`.
Enables detailed build logs from the bundler.
