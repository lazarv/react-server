...
**Per-render tokens** are generated each time a server component renders. Every render produces unique, unpredictable tokens because each encryption uses a random initialization vector. These tokens are embedded in the RSC stream and are used for inline server functions and server functions passed as props to client components.
...
- **Per-render uniqueness** — inline server function tokens are unique per render due to random initialization vectors, making them unpredictable across different renders of the same component.
