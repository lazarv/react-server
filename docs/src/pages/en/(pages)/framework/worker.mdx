...
- **Unified API:** The same `"use worker"` directive works in both server and client code. You write one module and the framework picks the right threading primitive for the environment.
...
- In development, the framework uses Vite's `ModuleRunner` inside the worker thread, providing full **Hot Module Replacement (HMR)** support. Changes to worker files are picked up automatically without restarting the dev server.
