...
Similar to Vite, `@lazarv/react-server` loads environment variables from `.env` and `.env.[mode]` files. Variable name starting with `VITE_*` or `REACT_SERVER_*` will be exposed as `import.meta.env.VITE_SOME_KEY` or `REACT_SERVER_SOME_KEY` and are available in client components and server components.
