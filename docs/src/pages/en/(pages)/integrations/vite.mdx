...
Similar to Vite, `@lazarv/react-server` loads environment variables from `.env` and `.env.[mode]` files. Variable name starting with `VITE_SOME_KEY` and `REACT_SERVER_` will be exposed as `import.meta.env.VITE_SOME_KEY` or `REACT_SERVER_SOME_KEY` and are availiable in client components and server components.
For more information checkout [Vite documentation](https://vite.dev/guide/env-and-mode) or the [react-server env example](https://github.com/lazarv/react-server/tree/main/examples/env) with a custom vite config.
