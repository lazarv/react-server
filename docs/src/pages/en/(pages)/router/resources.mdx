...
| `mapping` | yes | Function `({ params, search }) => key` that maps route parameters and search parameters to the resource key. Use `route.createResourceMapping(fn)` for type-safe params and search. |
