...
For file-system based routing, see the [file-system based router](/router/file-router) which builds on these primitives automatically. For the lower-level `Route` component API, see [Route component](#route-component) below.
...
On the server side, bind route descriptors to React elements and collect them into a router. The router provides a `<Routes />` component that renders the matched route.
