...
> I created `@lazarv/react-server` because I wanted to use React Server Components and Server Actions using Vite <span className="text-red-500">❤️</span>. For most small apps Next.js was too much and too heavy and slow. I wanted to have the same experience as you run a simple JavaScript file using node.js. This framework is trying to be non-opinionated as much as possible. You can achieve possibly anything you want. The only restriction is that it will use it's own React version. You don't even need to install React in your project. It's all included in the framework. I hope you will enjoy using this framework as much as I enjoyed creating it and using it to create this documentation too.
...
This framework is for you when you want to use bleeding-edge new technologies and you want a fast development process. When you're searching for a more minimalistic solution than a feature-packed framework like Next.js. When you want full control over how your application is built and how it works.
...
Advanced routing is hard to set up and to maintain, even worse when you need to refactor or restructure your application. File-system based routing is the best solution to this problem. So while the `@lazarv/react-server` framework is not including a file-system-based routing solution as a core component, you can install the `@lazarv/react-server-router` package to get this feature. This is to keep the core as small and un-opinionated as possible and to allow you to use any routing solution you want.
