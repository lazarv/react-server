...
We will also use the file-system based routing solution provided by `@lazarv/react-server-router` to create routes for our application. This will allow us to navigate between different pages and display the appropriate content.
...
cd photos
pnpm init
pnpm add @lazarv/react-server @lazarv/react-server-router react-click-away-listener @faker-js/faker
...
We enabled the `typescript-plugin-css-modules` plugin to support CSS modules in our project and we also include `.react-server/**/*.ts` files in our project to enable typed routing when using `@lazarv/react-server-router` with the built-in `Link` component.

When using the file-system based router we can specify the root directory for our routes in the `react-server.config.js` file so that the router knows where to look for the routes without crawling the entire project directory tree. This is especially useful for larger projects with many directories and files. Create the `react-server.config.js` file in the root of your project with the following content:
...
Using the `@lazarv/react-server-router` package and the file-system based routing, we no longer need to specify the entrypoint of our application. The router will automatically find the routes in the `src/app` directory and render the appropriate component for each route.
