...
サーバでエラーが発生した場合、クライアントでエラーが発生したわけではないので、エラーをリセットすることはできません。しかし、`Refresh`コンポーネントを使用することで、ページを再読み込みすることができます。詳しくは[ルーター](/ja/router)セクションの[クライアントサイドナビゲーション](/ja/router/client-navigation)ページを参照してください。
