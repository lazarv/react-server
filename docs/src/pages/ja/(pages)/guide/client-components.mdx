...
クライアントコンポーネントはサーバーコンポーネントからプロップスを受け取ることができます。プロップスはシリアル化された状態でクライアント側に受け渡されます。クライアント側に受け渡す全てのプロップスはシリアル化可能なものでなければいけません。プロップスとして基本的な値、配列、オブジェクトを渡すことができますが、関数を渡すことはできません。
