...
Viteのすべての設定は`vite.config.js`、`vite.config.ts`、`vite.config.mjs`、または`vite.config.cjs`ファイルに保存することができます。[Vite設定ガイド](https://ja.vitejs.dev/config/)に従ってViteを設定することができます。
...
- `rsc` - プラグインが React Server Components (RSC ペイロード) で使用される場合
...
RSCを使いたいので、`index.html`からすべてを`src/main.jsx`に移動し、クライアントサイドで`App`をレンダリングするのではなく、このRSCレイアウトでコンポーネントとして使うようにしましょう:
...
とりあえずApp.jsxに`"use client";`を追加するだけで、Appコンポーネントが`useState` Reactフックを含んでいるのですぐに動作するようになります。

では`react`と`react-dom`をアンインストールしましょう。これらはReact 18だったのでもう必要ありません。代わりに`@lazarv/react-server`をインストールする必要があります:
...
`App`コンポーネントから`"use client";`を削除し、`App.css`のインポートとカウンターコンポーネントの一部だけだったコードを削除します。その代わりに`Counter`コンポーネントをインポートしましょう:
...
これで両方のCSSファイルはページ読み込み時にすぐに読み込まれ、ちらつきがなくなります。カウンターボタンは以前と同様に動作し、クライアントコンポーネントとして新しくリファクタリングしたためです。`App`コンポーネントの他の部分は静的であり、RSCに含めるのが最適です。また2つのCSSファイルを1つに統合することもできます。2つある理由はないので、`main.jsx`でCSSのインポートを修正してください
