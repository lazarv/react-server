...
`ReactServerComponent`内には4種類の`Link`または`Refresh`ナビゲーションがあります。未指定、`target`、`local`、`root`です。`Link`または`Refresh`コンポーネントで`target`を使用すると、同じ名前のアウトレットのコンテンツのみが更新されます。`local`を使用すると、現在のページのURLは更新されず、親アウトレットのコンテンツが更新されます。`root`を使用すると、ページのコンテンツが更新されます。未指定の動作を使用すると、ページにアウトレットがある場合、各アウトレットのページのコンテンツが更新されます。
