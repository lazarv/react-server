...
タイプ固有のコンフィギュレータ関数を定義することで、異なるタイプのルートで同じ機能を使うこともできます。 `router` 関数はすべてのタイプのルートに対して呼び出されますが、異なるタイプのルートに対してルーティングを設定するために `layout`、`page`、`middleware`、`api` 関数を定義することができます。
