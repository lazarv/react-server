/**
 * CPU-profile the SSR (HTML) path for /client/large to find where
 * the @lazarv/rsc/client consumer spends time vs the old webpack consumer.
 *
 * Usage:
 *   node profile-ssr.mjs
 */
...
const PROFILE_REQUESTS = parseInt(process.env.PROFILE_REQUESTS || "200", 10);
const PATH = process.env.PROFILE_PATH || "/client/large";
const OUT = process.env.PROFILE_OUT || "/tmp/rsc-prof/ssr-perf.cpuprofile";
